...
Escrito após o redesign da landing e das telas de autenticação (cadastro/login). Deve crescer conforme as próximas telas (home, prédios, compromissos…) forem redesenhadas.
...
- **Campos de formulário** ([`RegistrarForm.vue`](../src/components/auth/RegistrarForm.vue), [`LoginForm.vue`](../src/components/auth/LoginForm.vue)): `prepend-inner-icon` para dar contexto ao campo; senha com toggle de mostrar/ocultar (`append-inner-icon` olho) em ambas as telas. A dica das regras de senha (`hint` `persistent-hint`, RNF04) aparece **só na criação** (cadastro), nunca no login. O `autocomplete` depende do contexto: `new-password` no cadastro, `current-password` no login (e `name`/`email` nos demais) — para os gerenciadores distinguirem criar de entrar.
