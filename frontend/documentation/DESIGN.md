...
- **FAB (ação flutuante)** — padrão da ação principal de uma tela da área logada: botão circular fixo no canto inferior direito. Na [`HomeView`](../src/views/home/HomeView.vue) é o **Logout** (`color="error"`, `mdi-logout`); nas telas filhas (próximos marcos) é a ação de "adicionar/criar/gerar" do módulo. No máximo um FAB por tela (ver [ARCHITECTURE.md](ARCHITECTURE.md), seção 5).
