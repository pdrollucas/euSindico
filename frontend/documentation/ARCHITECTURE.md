...
**Modelo de navegação dentro da área logada:** sem menu inferior persistente — a Home funciona como hub central (cards centralizados para Compromissos/Prédios/Configurações) e concentra também a ação de Logout (botão de largura total no rodapé da própria tela — não é mais um ícone fixo visível em toda rota protegida, ver decisão abaixo). Cada tela filha exibe um cabeçalho compartilhado (seta de voltar + título + atalho para Home) e seu próprio botão de ação flutuante (FAB — "adicionar"/"criar"/"gerar"); um menu inferior fixo abaixo desse botão ficaria visualmente ruim (botão em cima de botão).
